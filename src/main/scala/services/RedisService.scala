...
object CatsRedisServiceLive {
  ...
  val resource: Resource[IO, RedisCommands[IO, String, String]] =
    for {
      uri <- Resource.eval(RedisURI.make[IO](s"redis://$redisHost:$redisPort"))//Redis[IO].utf8(s"redis://${config.redis.password.value}@${config.redis.host}:${config.redis.port}/${config.redis.database}")
